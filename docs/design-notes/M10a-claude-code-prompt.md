# CLAUDE CODE PROMPT — M10a (spec-first: create the spec, then implement it)

> Copy everything below the line into Claude Code as a single prompt.

---

You are implementing **M10a — the score-as-energy growth economy (synchronous generational)**, the load-bearing invariant change of the v2 spine: population size becomes variable.

This is milestone-scale work and follows the spec-first workflow in `CLAUDE.md`. **First write the spec file, then implement it.**

Read before starting: `docs/DESIGN.md` (§2.7, §3.1, §4, §5, §6, §8), `docs/DECISIONS.md` (especially **#10, #20, #22, #23, #31, #32, #34, #35, #38, #44, #46, #47, #48, #57, #58, #60, #61, #62, #63, #64, #65, #75**), `docs/ROADMAP.md`, and `docs/explainers/M9.5-sweeps-and-invasion.md` (for the explainer voice you will match later).

The design was frozen in a Claude.ai design conversation. **The spec below is that freeze.** Deviations you discover during implementation are logged in `docs/DECISIONS.md` — the spec is **not** retro-edited beyond its status line (#62).

## Step 1 — create `docs/specs/M10a-growth-economy.md`

Write the file with exactly the content in the fenced block below. Two carve-outs, and only these two:

- **Task 0 is a verify-first task.** If your reading of the code contradicts a factual claim the spec makes about the *current* code, do not silently write the spec's version. Follow the spec's stated fallback, and log the divergence in DECISIONS.
- Fix any typo you spot. Do not restructure, re-scope, or "improve" the design.
...
Companion explainer: `docs/explainers/M10-growth-economy.md` (the science, the
worked arithmetic, and the literature grounding — read it for the *why*). This
spec is the *what* and *how*.
...
Eleven new parameters, section `"Dynamics"`. Every one carries a plain-language
...
`dynamics.generations` and BEFORE `dynamics.selection_rule`**. The remaining ten
follow the accounting parameters, at the end of the Dynamics block.
...
- define `_ECONOMY_PARAMS` (the ten economy knobs) and `_IMITATION_PARAMS` (the
  five selection keys + the three accounting keys);
...
- **`docs/explainers/M10-growth-economy.md`**: the companion explainer arrives
  as a separate prompt — create the file only if that prompt has already landed;
  otherwise leave the spec's pointer dangling and say so in your report.
...
Work the tasks in order. Task 0 first, and **report what you found** before
touching anything else.

## Step 3 — DECISIONS (append-only, `docs/DECISIONS.md`)

The log currently ends at **#75**. Allocate **sequentially from #76**. Each entry:
number, date, decision, rationale, alternatives considered. The intended cluster
— merge or split as the work actually lands, but keep them sequential and
**report the final numbers**:

- **#76** — Milestone renumbering. Carries the table. States explicitly that it
  supersedes the **numbering** — *not* the substance or rationale — of **#58**
  and **#75**. Append-only: #58 and #75 are **NOT** retro-edited.
- **#77** — M10 splits into M10a (synchronous) / M10b (async). Energy is a
  distinct paradigm that **replaces** imitation; the SelectionRule and
  ScoreAccounting families grey out (#34). Ties off #64's deferred `cumulative`
  ...
  spends". Different jobs → replace, don't compose. Rejected:
  one-milestone-both-modes; async-first.
- **#78** — The open-flow ledger (sources / sinks / transfers) and the two
  ...
  match-length knobs silently *economic*, and under `w` would inherit a random
  match length → an RNG-entangled cost. Rejected: coupling the two costs by a
  ratio (a units problem M10 itself creates — energy/generation vs energy/match
  needs a match count, but N changes every generation by design; it also breaks
  M9.5 axis independence).
- **#79** — Offspring policy: stake transfer, σ ≤ θ, one birth per agent per
  generation, `initial_energy` defaults to σ. Rejected: fixed endowment; zero
  endowment (folded in as σ = 0); binary fission.
- **#80** — Death at `e < 0` and the M10a per-generation draw sequence
  (extends #32). **Death-before-birth is a plain design preference and deviates
  from Hammond–Axelrod's birth-before-death period order** — name the deviation
  honestly; do **not** claim it is "spatially correct for M11".
- **#81** — `capital_return_rate`; escape velocity `e* = costs / r`; capital
  return + highest-energy-first admission = **structurally permanent dynasty**,
  named as a mechanism rather than buried as a footnote.
- **#82** — `carrying_capacity`: aspatial-specific scope; deterministic
  RNG-free highest-energy-first admission; placement and admission isolated in
  two named free functions (no speculative ABC, hard rule 6);
  check-placement-then-pay-σ.
- **#83** — The mortality trio and the registry's **first derived defaults**
  (`None` + `nullable` as the sentinel, reusing the `population.memory_depth`
  precedent rather than an `"auto"` string); resolved in a `mode="before"`
  validator because the models are `frozen=True`; warn-don't-forbid on effective
  max age.
- **#84** — Founder age staggering (automatic, no parameter).
- **#85** — Passport ids (never reused, monotonic) + `parent_id` lineage;
  `total_agents_born` / `population_final`.
- **#86** — The variable-N `random_k` contract: clamp to `min(k, N−1)`; the
  clamp is a verified no-op at N ≥ k+1 so no existing seeded run changes; the
  lone-survivor corner; id-sorted iteration over a non-contiguous id set;
  extinction ends the run.
- **#87** — Per-agent snapshot on `GenerationFinished`, economy-only,
  post-boundary carried-forward grain (with its named cost); population size
  derived not carried; no explicit birth/death events (deferred to M10b).
- **#88** — `schema_version` 3 + the `agents.parquet` sibling; loaders accept
  {1, 2, 3}; imitation runs still write 2. Rejected: widening
  `timeseries.parquet` (#47c).
- **#89** — The calibration readout ships **in** M10a (app-first validation is
  not honest without it).
- **#90** — Per-agent `matches_played` tally (whatever Task 0a found).
- **#91** — Per-opponent histories **persist across generations** in economy
  mode; score-only reset at the boundary. Amends the **scope** of **#31** (whose
  rationale is selection-specific and dissolves when nobody's strategy is
  overwritten and ids are never reused) and of **#22** (`round_number` is
  lifetime-cumulative in economy mode); follows the **#34** tournament precedent,
  where persistent agents already keep persistent grudges by design. Names the
  consequences: GrimTrigger is lifetime-grim; `Agent.rounds_played` becomes a
  lifetime count in economy mode and must not feed #44's per-generation
  denominator (see #90's tally). Names the cost: `view_of`'s O(length²) copy
  becomes quadratic in run length under round_robin; `memory_depth` is the
  existing bound; the calibration readout warns rather than forbids. Rejected:
  per-generation reset — it makes an agent that remembers its bank balance but not
  its enemies, and its stated rationale does not survive the move away from
  imitation.

## Step 4 — finish

Run `ruff check . && ruff format .` and `pytest`. Then perform the **mandatory
end-of-session ritual** from `CLAUDE.md`:

- `DOCS CHANGED: <list>` — call out the new DECISIONS entry numbers explicitly.
- Show the **app-first manual validation** steps (with the venv reminder).
- Present (a) a summary of what was done, (b) the list of files to stage, and
  (c) a suggested commit message. **Do not run `git commit`** — the commit is the
  owner's act.

Action required: paste this prompt into Claude Code, answer its Task 0 findings report if it flags a divergence, then perform the git commit yourself once it presents the file list and suggested message.
