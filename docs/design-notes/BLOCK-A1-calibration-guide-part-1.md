...
> text editor, or use the raw view, not a rendered preview. Do not retype it.
> This is part 1 of 2; Block A2 supplies §7 onward.

---

# PROMPT FOR CLAUDE CODE — BLOCK A1 of 3

## Cleanup first

A previous prompt was truncated in transit and its formatting was stripped. Before doing anything else:

1. Delete `docs/explainers/calibration-guide.md` entirely if it exists.
2. `docs/WIP.md` was read and deleted by the previous run. **That was correct and needs no repair** — the file is untracked and ephemeral by design, its content was duplicated in tracked docs, and its status was stale. Do not attempt to restore it. Instead, confirm in one line that the M11a Phase B entry point and the outstanding verification tasks are still recoverable from `docs/specs/M11a-population-structure-spec.md` and `CLAUDE.md`, and say so in your handback. If they are not, report that as a gap rather than reconstructing anything.
3. Ignore the preserved scratchpad copy of the earlier truncated text at `…/scratchpad/calibration-guide-block-a-as-received.txt`. It is defective in two ways and must not be used as a source. You may delete it once this prompt completes.

Report what you found for steps 1 and 2 before proceeding.

## The task

Create one new file, `docs/explainers/calibration-guide.md`, containing **exactly** the content in the fenced block below — the file content is everything between the ```` ```markdown ```` fence and its closing fence.

This is a standalone explainer with no companion specification. That deviation is deliberate and is logged separately in Block B, so do not create a spec for it.

**Stop where the content stops**, at the end of §6.4. The file is intentionally incomplete. Sections §7, §8, §9 and the References arrive in Block A2, which appends to this same file.

Do not:

- add a placeholder, a table of contents, or any note about the missing sections
...
*(This table is derived from the behaviour documented in the parameter registry, which is authoritative for what each rule is meant to do. The `threshold_cloning` row in particular is a claim about arithmetic; a code check is pending.)*
...
When you have created the file, report:

1. What steps 1 and 2 of the cleanup found.
2. The file path and its line count.
3. Confirmation that the final line of the file is exactly: `Miss one and the invariance is gone.`
4. Confirmation that the `e\*` in §6.3 survived as a literal asterisk (it should render as `e*`, not as italics).

Do not commit. Do not modify any other file.

**Action required:** Perform the cleanup, create `docs/explainers/calibration-guide.md` with exactly the fenced content above, stop at §6.4, and report the four items listed without committing.
