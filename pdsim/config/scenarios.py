...
register_scenario(
    ScenarioInfo(
        ...
        description=(
            ...
            "and cooperation's share can rise early. And on the shipped "
            "seed that is exactly what happens: the grid FILLS. A seat "
            "under the carrying capacity goes only to a parent that has "
            "an empty site within its birth reach (Moore radius 1) — the "
            "capacity decides HOW MANY children are born, the birth "
            "radius decides WHERE — so a rich interior parent with no "
            "room around it is simply not ranked for a seat, and the "
            "frontier does the growing. Observed, generation by "
            "generation: 60 → 85 → 112 → 144 → 181 → 227 → 272 → 313 → 346 "
            "→ 380 → 390 → 396 → 399 → 400 — full after 13 generations, "
            "with not a single death along the way, and cooperation's "
            "share rising from 0.50 to 0.625 (250 cooperators, 150 "
            "defectors). Watch the two Economy readouts as it fills: "
            "'Infeasible parents this generation' — rich enough to breed "
            "but no empty site in reach, the closing interior — climbs "
            "from 32 toward the whole population, while 'Blocked parents "
            "this generation' — an earlier-placed parent took the last "
            "empty site in reach — stays small, between 1 and 18, and "
            "drops to 0 once the grid is full. Then the endgame is a "
            "STANDSTILL BY SATURATION: with all 400 sites occupied every "
            "eligible parent is infeasible (the readout shows 400, the "
            "whole population, every generation), nobody can be born, "
            "and on this seed nobody dies. The arithmetic says why: on "
            "the full torus every agent plays 16 matches of 10 rounds; a "
            "defector among defectors earns 16 × 10 × 1 = 160, a defector "
            "beside cooperators up to 16 × 10 × 5 = 800, and a cooperator "
            "with even ONE cooperator neighbour at least 2 × 10 × 3 = 60 "
            "— all against the living cost of 40. Only a cooperator with "
            "NO cooperator neighbour earns 0 and would starve, and since "
            "children land beside their parents no such cooperator "
            "exists here. Zero deaths, zero births, 250 to 150 from the "
            "moment the grid fills to the end of the run: 300 generations "
        ...
        things_to_try=(
            "Add a little churn: set 'Base hazard' in the Dynamics "
            "section to 0.02 and re-run. The arithmetic you can check: "
            "about 0.02 × 400 = 8 deaths per generation once the grid is "
            "full, each freeing one site that only its neighbours can "
            "breed into — so the standstill turns into a slow contest for "
            "vacated sites, seat by seat. Who wins each seat is decided by "
            "wealth among the FEASIBLE parents (the richest with an empty "
            "site in reach are seated first), and incomes differ sharply "
            "by neighbourhood: an interior cooperator earns 16 × 10 × 3 = "
            "480 per generation, an interior defector 160, and a defector "
            "on the edge of a cooperator cluster up to 16 × 10 × 5 = 800. "
            "Whether cooperator clusters grow or erode at their edges is "
            "something to watch, not something promised. And recompute "
            "the survival window whenever you touch a payoff or the "
