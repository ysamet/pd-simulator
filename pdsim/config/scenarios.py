...
register_scenario(
    ScenarioInfo(
        ...
        description=(
            ...
            "and cooperation's share can rise early. But on the shipped "
            "seed saturation never arrives, and what actually happens is "
            "the scenario's real lesson: RISE, THEN FREEZE. Reproduction "
            "here has two gates, and a parent must clear both — clearing "
            "one is not clearing the other. The first gate is global "
            "admission: births only fill seats left under the carrying "
            "capacity, and when seats are scarce the RICHEST eligible "
            "parents are admitted first. The second gate is local "
            "placement: an admitted parent must find an empty site within "
            "reach (Moore radius 1) to put the child in — and a parent "
            "that fails placement pays nothing and stays eligible, and "
            "richer, next generation. At population scale the two gates "
            "deadlock: the richest parents are the all-cooperator "
            "interior (earning 480 per generation, compounding), and "
            "interior agents are precisely the ones with no empty site in "
            "reach — so they consume the entire admission quota and then "
            "fail placement, every generation, while the poorer frontier "
            "parents, every one of them above the breeding threshold and "
            "beside an empty site, never rank inside the quota. Growth "
            "stops around 265 of the 400 sites with zero deaths and "
            "near-zero births: a genuine standstill, not a slowdown. See "
            "the signature yourself in the Economy panel: from about "
            "generation 6 onward, 'Blocked parents this generation' "
            "equals EXACTLY 400 minus the population, every generation — "
            "the whole admission quota, spent on parents who cannot "
            "place. (Had the grid filled, the expected endgame was a slow "
            "grind — a saturated defector interior earns 160 against the "
            "living cost of 40 and never starves, so only fully-encircled "
            "cooperators could die, one cell at a time — but the freeze "
            "arrives first.) 300 generations gives the rise room to "
            "complete and the freeze room to prove it is permanent."
        ),
        config=ExperimentConfig.model_validate(
            {
                "seed": 11,
        ...
        things_to_try=(
            "Switch the punishment payoff P to 0 — after first unticking "
            "'Enforce PD payoff ordering (T > R > P > S)' in the Game "
            "section, because the sucker payoff here is 0, so P = 0 ties "
            "P and S and the strict-ordering check would reject it — and "
            "re-derive before running: a defector whose occupied "
            "neighbours are all defectors now earns 16 × 10 × 0 = 0 "
            "against the living cost of 40, and starves. Dead defectors "
            "free interior sites — and freed interior sites are exactly "
            "what the frozen world lacks, so the freeze MAY break and "
            "the fill resume. That is arithmetic, not a promise: whether "
            "enough defectors sit in all-defector pockets to matter is "
            "something to watch, not something guaranteed. And recompute "
            "the survival window whenever you touch a payoff — this one "
            "change moves the all-defector income bound from 160 to 0."
        ),
    )
)
